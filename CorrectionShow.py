...
def doc_scanning(imgdir,outdir):
    ...
    with tf.Session() as sess:
        ...
        #     for j in line_mean:
        #         x1, y1, x2, y2 = j
        #         xf, yf = getFootPoint(x0, y0, x1, y1, x2, y2)
        #         if xf < 0 or xf > w or yf < 0 or yf > h:
        #             continue
        #         dis = round(sqrt((x0 - xf) ** 2 + (y0 - yf) ** 2), 2)
        #         pointd = [x1, y1, x2, y2, dis]
        #         group[n].append(pointd)
        #     # print(group[n])
        #     # print('~~~~~~~~~~~~~~~~~~~~~')
        #     group[n] = sorted(group[n], key=lambda x: x[-1])
        #     if len(group[n]) >= 3:
        #         group[n] = group[n][:3]
        #     n += 1
        # print('group is :\n',group)
        ...
        #     img_extd = cv2.cvtColor(np.asarray(im), cv2.COLOR_RGB2BGR)
        #     for j in group[i]:
        #         x1n, y1n, x2n, y2n = j[0:4]
        #         # print(x1n, y1n, x2n, y2n)
        #         cv2.line(img_extd, (x1n, y1n), (x2n, y2n), (255, 0, 0), 1)  # BGR
        #     img1 = Image.fromarray(cv2.cvtColor(img_extd, cv2.COLOR_BGR2RGB))
        #     savepath = os.path.join(outdir, os.path.basename(os.path.splitext(name)[0]) +'_nearestline'+'_'+str(i)+'_'+ '.jpg')
        #     img1.save(savepath)
        #     img2 = Image.open(savepath)
        #     img2.show()
        #
        ...
        # print('~~~~~~~~~~~~~~~~~~~~ intersecs')
        #
        # # 12 13 14 23 24 34依次找交点
        # intersecs = []
        # for i in comb4:
        #     for n1 in range(3):
        #         x1, y1, x2, y2 = i[n1][0:-1]
        #         for n2 in range(n1 + 1, 4):
        #             x3, y3, x4, y4 = i[n2][0:-1]
        #             [xx, yy] = findIntersection(x1, y1, x2, y2, x3, y3, x4, y4)
        #             #             print(xx,yy)
        #             if xx > 0 and xx < w and yy > 0 and yy < h:
        #                 if [xx, yy] not in intersecs:
